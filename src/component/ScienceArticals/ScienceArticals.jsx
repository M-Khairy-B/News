...
import { fetchentScienceData } from "../../lib/scienceSlice";

export default function ScienceArticals() {
    ...
    const { scienceData, status, error } = useSelector(
        (state) => state.scienceData
    );
    useEffect(() => {
        dispatch(fetchentScienceData({ category, country }));
    ...
    useEffect(() => {
        if (scienceData) {
            setIconStyle(new Array(scienceData.length).fill("regular"));
            setIconStyleChange(new Array(scienceData.length).fill("regular"));
        }
    }, [scienceData]);
    ...
    const handleViewMore = () => {
        if (showAll) {
            ...
            setVisibleCount(scienceData.length);
        }
        setShowAll(!showAll);
    };
    ...
    return (
        <React.Fragment>
            <section className="all-article-main general">
                <div className="container">
                    <div className="text-article-main">
                        <h2 className="text-mains">Science</h2>
                    </div>
                </div>
