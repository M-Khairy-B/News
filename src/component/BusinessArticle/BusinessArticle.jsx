...
import { fetchBusinessData } from "../../lib/businessSlice";

export default function BusinessArticle() {
    ...
    const { businessData, status, error } = useSelector(
        (state) => state.businessData
    );
    useEffect(() => {
        dispatch(fetchBusinessData({ category, country }));
    ...
    useEffect(() => {
        if (businessData) {
            setIconStyle(new Array(businessData.length).fill("regular"));
            setIconStyleChange(new Array(businessData.length).fill("regular"));
        }
    }, [businessData]);
    ...
    const handleViewMore = () => {
        if (showAll) {
            ...
            setVisibleCount(businessData.length);
        }
        setShowAll(!showAll);
    };
            ...
            <section className="all-article-main business">
                <div className="container">
                    <div className="text-article-main">
                        <h2 className="text-mains">Business</h2>
                    </div>
                </div>
