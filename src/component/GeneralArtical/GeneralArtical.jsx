...
import { fetchentGeneralData } from "../../lib/generalSlice";

export default function GeneralArtical() {
    ...
    const { generalData, status, error } = useSelector(
        (state) => state.generalData
    );
    useEffect(() => {
        dispatch(fetchentGeneralData({ category, country }));
    ...
    useEffect(() => {
        if (generalData) {
            setIconStyle(new Array(generalData.length).fill("regular"));
            setIconStyleChange(new Array(generalData.length).fill("regular"));
        }
    }, [generalData]);
    ...
    const handleViewMore = () => {
        if (showAll) {
            ...
            setVisibleCount(generalData.length);
        }
        setShowAll(!showAll);
    };
            ...
            <section className="all-article-main general">
                <div className="container">
                    <div className="text-article-main">
                        <h2 className="text-mains">General</h2>
                    </div>
                </div>
