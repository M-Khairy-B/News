...
import { fetchSportData } from "../../lib/sportSlice";

export default function SportArticle() {
    ...
    const { sportData, status, error } = useSelector(
        (state) => state.sportData
    );
    useEffect(() => {
        dispatch(fetchSportData({ category, country }));
    ...
    useEffect(() => {
        if (sportData) {
            setIconStyle(new Array(sportData.length).fill("regular"));
            setIconStyleChange(new Array(sportData.length).fill("regular"));
        }
    }, [sportData]);
            ...
            <section className="all-article-main business">
                <div className="container">
                    <div className="text-article-main">
                        <h2 className="text-mains">Sports</h2>
                    </div>
                </div>
