...
import { fetchTechnologyData } from "../../lib/technologySlice";
export default function TechnologyArticles() {
    ...
    const { technologyData, status, error } = useSelector(
        (state) => state.technologyData
    );
    useEffect(() => {
        dispatch(fetchTechnologyData({ category, country }));
    ...
    useEffect(() => {
        if (technologyData) {
            setIconStyle(new Array(technologyData.length).fill("regular"));
            setIconStyleChange(
                new Array(technologyData.length).fill("regular")
            );
        }
    }, [technologyData]);
    ...
    const handleViewMore = () => {
        if (showAll) {
            ...
            setVisibleCount(technologyData.length);
        }
        setShowAll(!showAll);
    };
    ...
    return (
        <React.Fragment>
            <section className="all-article-main technology">
                <div className="container">
                    <div className="text-article-main">
                    <h2 className="text-mains">Technology</h2>
                    </div>
                </div>
